...
def serve():
    ...
    # 设置socket监听端口
    server.add_insecure_port('192.168.226.1:30000')
    # 启动grpc server
    server.start()
...
class Telemetry_CPU_MEM_Info(
    ...
    def dataPublish(self, request_iterator, context):
        print("context:", context)  #   context: <grpc._server._Context object at 0x000001E1E38EC6A0>
        for i in request_iterator:
            ...
            for row_data in telemetry_data.data_gpb.row:
                print("row_data:", row_data.content)
                print('-----------------')
                ...
                print(decode_func)
                print(getattr(decode_module, root_class))
